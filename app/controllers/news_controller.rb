class NewsController < ApplicationController
  ...
  def setup
    ...
    if params[:collaboration_id]
      ...
      @new_task.collaboration_id = @collaboration.id
      @new_task.topic_id = @collaboration.unfiled_topic.id
    end
    if params[:topic_id]
      @topic = Topic.find(params[:topic_id])
      @new_task.topic_id = @topic.id
    end
    if params[:collaboration_user_id]
      @collaboration_user = CollaborationUser.find(params[:collaboration_user_id])
      @new_task.assigned_to = @collaboration_user.user_id
    end
